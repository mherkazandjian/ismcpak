...
c     Start with summary of input parameters
 ...
 32   format (a)
      ...
      write(*,32) '* Radex version        : '
     $      //version(1:length(version))
      if (method.eq.1)
     $write(*,32) '* Geometry             : Uniform sphere'
      if (method.eq.2)
     $write(*,32) '* Geometry             : Expanding sphere'
      if (method.eq.3) 
     $write(*,32) '* Geometry             : Plane parallel slab'
c      write(8,32) '* Molecular data file  : '//specref(1:80)
      write(*,32) '* Molecular data file  : '//molfile(1:100)
      write(*,30) '* T(kin)            [K]: ',tkin
c      write(8,31) '* Total density  [cm-3]: ',totdens
      if(density(1).gt.eps)
     $write(*,31) '* Density of H2  [cm-3]: ',density(1)
      if(density(2).gt.eps)
     $write(*,31) '* Density of pH2 [cm-3]: ',density(2)
      if(density(3).gt.eps)
     $write(*,31) '* Density of oH2 [cm-3]: ',density(3)
      if(density(4).gt.eps)
     $write(*,31) '* Density of e-  [cm-3]: ',density(4)
      if(density(5).gt.eps)
     $write(*,31) '* Density of H   [cm-3]: ',density(5)
      if(density(6).gt.eps)
     $write(*,31) '* Density of He  [cm-3]: ',density(6)
      if(density(7).gt.eps)
     $write(*,31) '* Density of H+  [cm-3]: ',density(7)
      write(*,30) '* T(background)     [K]: ',tbg
      write(*,31) '* Column density [cm-2]: ',cdmol
      write(*,30) '* Line width     [km/s]: ',deltav/1.0d5

      write(*,33) 'Calculation finished in ',niter,' iterations'
 33   format(a,i7,a)
...
c     Line flux in K*cm/s and in erg/s/cm^2 
          if (dabs((tex(iline))).lt.1000.0) then
            write(*,113) qnum(m),qnum(n),eup(iline),spfreq(iline),wavel,
     $         tex(iline),taul(iline),ta,xpop(m),xpop(n),kkms/1.0d5,ergs
          else
            write(*,114) qnum(m),qnum(n),eup(iline),spfreq(iline),wavel,
     $         tex(iline),taul(iline),ta,xpop(m),xpop(n),kkms/1.0d5,ergs
          endif
 113      format(a, '  ', a,f12.1,2(2x,f12.4),1x,f8.3,6(1x,1pe15.3e3))
 114      format(a, '  ', a,f12.1,2(2x,f12.4),7(1x,1pe15.3e3))
	endif
      enddo
