...
class species(numpy.recarray):
    '''A class for defining species. If the species is a baseSpecie (i.e it is not derived and made 
     up of other species, then its field 'derived' should be set to 0, the all the keywords 
     (except comp) are mandatory.
     
    .. note:: the maximum allowed length of a species name can be set by changing the value 
     of max_spec_name_str_len in __new__()
     
    .. todo:: set the default stuff!!! while intializing the objects  
    '''
    def __new__(cls, shape=None, name=None, derived=None, charge=None, init=None, 
                     comp=None, mass = None, num=None,
                     *args, **kwargs):

        #the maximum allowed length of a species name string
        max_spec_str_len = 20
        ...
        descr = [
                 ('name', 'S%i' % max_spec_str_len), ('derived', 'b'), ('charge', 'i4'), 
                 ('init', 'b'), ('mass' , 'f8'), ('passive', 'b'), ('num', 'i4'),
                ]
       
        return numpy.recarray.__new__(cls, shape, dtype = numpy.dtype(descr) )

    def __init__(self, shape=None, name=None, derived=None, charge=None, init=None, 
                     comp=None, mass = None, passive = None, data=None, num=None, *args, **kwargs):

        if data == None:
            if name    != None: self.name    = name
            if derived != None: self.derived = derived 
            if charge  != None: self.charge  = charge 
            if init    != None: self.init    = init 
            if mass    != None: self.mass    = mass
            if passive != None: self.passive = passive
            if num     != None: self.num     = num
            if comp    != None: self.comp    = comp 
        else:
            for field in self.dtype.fields: self[field] = data[field]

        self.comp = lil_matrix(self.size, self.size, 'i4')

        
    def pprint(self):    
        '''method that prints the string representation of the specie'''
        
        print '    name             |  charge    number   derived   initialized     passive          mass       components'
        print '---------------------+-------------------------------------------------------------------------+-----------'
        
        for i in numpy.arange(self.size):
            s = self[i]
            
            print '%-+20s |' % s.name,
            
            chargeStr  = '%-d'  % s.charge  if numpy.isfinite(s.charge)  else 'NA'
            numStr     = '%-d'  % s.num     if numpy.isfinite(s.num)     else 'NA'
            derivedStr = '%-d'  % s.derived if numpy.isfinite(s.derived) else 'NA'
            initStr    = '%-d'  % s.init    if numpy.isfinite(s.init)    else 'NA'
            passiveStr = '%-d'  % s.passive if numpy.isfinite(s.passive) else 'NA'
            massStr   = '%.7e' % s.mass    if numpy.isfinite(s.mass)    else 'NA'
            ...
            print '    %s   %s    %s          %s              %s       %s |' % (chargeStr, numStr, derivedStr, initStr, passiveStr, massStr)
