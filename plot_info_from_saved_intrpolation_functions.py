"""
<keywords>
interpolation functions, scipy, ismcpak, radex, plot
</keywords>
<description>
...
if False:
    gms = [-50.0, -30.0, -25.0, -23.0, -22.5, -22.0,]
    Av  = 10.0 
    G0  = 1.0
    ...
    for gm in gms:
        ...
        logG0s = numpy.ones(logns.shape, 'f8')*2.0
        logGMs = numpy.ones(logns.shape, 'f8')*gm
        Avs    = numpy.ones(logns.shape, 'f8')*10.0
        
        data = numpy.array([logns, logG0s, logGMs, Avs]).T
        v = F.get( data )
        
        pylab.plot(logns, v, '-', label=gm)
