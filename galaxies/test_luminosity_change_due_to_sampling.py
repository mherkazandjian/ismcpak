...
params = {
          ...
          'weights' : 'conserve-area', #'by-number', #'matched',  #'original-only' ,#None ,#'conserve-area'
          'obs_res'      : 21,
...
## copying the parents in to a new set
gasp = gas[gas.get_inds_has_children()] 
## setting the parents back to their original values
gasp.set_radii(weighting='original-only', rundir=params['rundir'], snap_index=params['snap_index'])

fi_utils.effective_area_tests(gas, gasp, params)
